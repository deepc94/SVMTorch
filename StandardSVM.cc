...
void StandardSVM::train(real **data_, sreal **sdata_, real *target_, int l_, int n_input_dim_)
{
  ...
  // I pray that the normal user uses
  ...
  /////
  static int supVecCount = 0;
  supVecCount += n_support_vectors;
  cout << "# " << n_support_vectors << " support vectors\n";
  cout << "# With " << n_support_vectors_bound << " support vectors at C\n";
  cout << "# Cumulative Number of Support Vectors: " << supVecCount << endl;
  // ofstream acc("supVec.txt", ios::out | ios::app);
  // acc << n_support_vectors <<"  "; 
  // acc.close();
...
void StandardSVM::save(string file, string comment)
{
  ...
  string fname = file + "_ascii.txt";
  ofstream f(file.c_str(), ios::out | ios::trunc | ios::binary);
  ofstream asc(fname.c_str(), ios::out | ios::trunc);

  if(!f || !asc)
  {
    cout << "$ File error. Arg. 1\n" << endl;
    exit(0);
  }
...
//==============================================================================//
  asc << "# " << VERSION << endl;
  asc << "# " << comment << endl;
  asc << "# " << n_support_vectors << " support vectors inside" << endl;
  asc << "# With " << n_support_vectors_bound << " support vectors at C" << endl;
//==============================================================================//
 ...
 //================================================//
  asc << "regression mode: " << regression_mode;
  asc << "\t sparse mode: " << sparse_mode << endl;
//================================================//
...
//===================================================================//
  asc << "\nno. of support vectors: " << n_support_vectors << endl;
  asc << "input dimension: " << n_input_dim << endl;
  asc << "b = " << b << endl;
  asc << "\n";
//===================================================================//
  if(sparse_mode)
...
///=============////////////////////////////////////////
//                                                    //
//    Edited by: Deep Chakraborty                     //
//    Date edited: 05.07.2015                         //
//                                                    //
////////////////////////////////////////==============//
...
//===============================================================//
    ...
    if (kernel->id == 4)
    {
      asc <<"alpha * y \t support vector indices"<<endl;
      asc <<"================================\n"<<endl;
      for(int it = 0; it < n_support_vectors; it++)
      {
        f.write((char *)&sv_alpha[it], sizeof(real));
        f.write((char *)&support_vectors[it], sizeof(int));
        asc << sv_alpha[it] << " \t\t " << support_vectors[it] <<endl;
      }
    }
    else
    {
      asc <<"alpha * y \t data address"<<endl;
      asc <<"=================================\n"<<endl;
      for(int it = 0; it < n_support_vectors; it++)
      {
        int t = support_vectors[it];
        ...
        asc << sv_alpha[it] << " \t\t " << data[t%lm] << endl;
      }
    }
  }
  asc.close();
  ...
  // Kernel load
  {
    ...
    switch(id)
    {
      ...
      case 4:
        ...
        break;
      default:
        cout << "$ What the hell kinda kernel is this ?!?\n\n";
        exit(0);
    }

    delete[] u;
  }
  //cout<<"Kernel Id is: "<<kernel->id<<endl;
...
//===============================================================//
    if(kernel->id == 4)
    {
      ...
      for(int t = 0; t < n_support_vectors; t++)
      {
        ...
        f.read((char *)&support_vectors[t], sizeof(int));
      }
    }
    else
    {
...
//====================================================//
  if(kernel->id == 4)
  {
    ...
    {
      int t = support_vectors[it];
      sum += sv_alpha[it]*kernel->evalue(x, t);
    }
  }
  else
  {
